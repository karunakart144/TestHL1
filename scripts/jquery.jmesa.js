    ...
    var filterapi = {
        ...
        saveDynFilterData : function(cell, input, originalValue) {
            ...
            if(originalValue != changedValue) {
            	//Added to filter date values according to dd-MM-yyyy format
            	if(dynFilter.property=='LOCKED_DATE' || dynFilter.property=='CREATED_DATE'){
            		//Regular Expression for date in dd-MM-yyyy format
            		 var regExpr=/^(\d{1,2})-(\d{1,2})-(\d{4})$/;
            		 //Regular Expression to remove white space characters
            		 changedValue=changedValue.replace(/^\s*|\s*$/g,'');
                     if(changedValue.match(regExpr))
                     {
